...
export default function MenuButton({ openSideBar, showSideBar }: props) {
    ...
    return (
        <button onClick={handleClick} className="flex items-center gap-2 hover:bg-orange-500 px-4 py-2 md:px-8 md:py-2 rounded" >
            <div className="relative">
                <span ref={ref} className={`${style.hamburgerIcon} ${showSideBar ? style.active : null}`}></span>
            </div>
            <span className="hidden md:inline capitalize font-bold text-2xl">menu</span>
        </button>
    )
}